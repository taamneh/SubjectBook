...
import java.io.{File, IOException, InputStream, PrintWriter}
...
object AuxiliaryMethods extends Controller {
      ...
      if(!lst.contains(subjectInfo.getTitle)) {
        ...
        for ((sessionId, sessionInfo) <- sessions) {
          ...
          sessionName = sessionName match {
            case "Baseline" => "BL"
            case "PracticeDrive" => "PD"
            case "NormalDrive" => "ND"
            case "MotoricDrive" => "MD"
            case "CognitiveDrive" => "CD"
            case "FinalDrive" => "FDL"
            case _ => sessionName
          }
  ...
  def MalcomREportDeleteAFter (study : String) ={
    ...
    val subjects = GoogleDrive.returnFilesInFolderJustForTest2(service, "0BzuoB3uc0tTVa2F6ZkViY3lDcHc", "mimeType = 'application/vnd.google-apps.folder'")
