  ...
  */
  ...
  def drp(studyNo: Int) = Action {
    implicit request =>
      ...
      var location  = AuxiliaryMethods.ReportDataInDrPFormat(studyNo) ;
      Ok.sendFile(
      ...
      val videosTemp = SQL("select session_name,signal_loc, file_name from session, signals  where signal_signal_code = signal_code AND data_type = 2 AND subject_seq = (select subject_seq from subject where subject_id ={sub_id}  AND study_id ={study_no});").
        ...
        if(hide == 8){
          ...
          for((k,v) <- videosPerSession){
            videosPerSession += k -> List((k , "0BzuoB3uc0tTVNGJjNUFQX09CYU0", ""))
          }
        }
  ...
  def getVideo () = Action {
    implicit request =>
      ...
      var loc = ProductionSide.videosLocattion + "T074-008.avi1.avi.mp4"
  ...
  def test (studyNo: Int) = Action {
    implicit request =>
     ...
     // AuxiliaryMethods.RebuildRealTimeStudy(studyNo)
